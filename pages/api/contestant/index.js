...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const exists = await Contestant.findOne({ email: data.email });
      if (exists) return res.status(409).json({ error: 'Email already registered.' });

      const contestant = new Contestant({ ...data });
      await contestant.save();
  ...
  if (req.method === 'GET') {
    try {
      ...
      const list = await Contestant.find(query).sort({ createdAt: -1 });
  ...
  if (req.method === 'DELETE') {
    try {
      ...
      if (ids) {
        await Contestant.deleteMany({ _id: { $in: ids.split(',') } });
        return res.status(200).json({ message: 'Deleted selected' });
      }
